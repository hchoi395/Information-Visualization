...
function updateBars(house) {
    ...
    if (house === 'top') {
        filteredCharacters = characters.filter(character => character.power_ranking > 0);
    } else {
        filteredCharacters = characters.filter(character => character.house === house);
    }

    filteredCharacters = filteredCharacters.sort((a, b) => b.probability_of_survival - a.probability_of_survival);
